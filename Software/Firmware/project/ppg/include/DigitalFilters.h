...
constexpr T pow(T input, unsigned int power) {
  return (power == 0 ? 1
                     : input * (power <= 1 ? 1 : tps::pow(input, power - 1)));
}
}  // namespace tps
...
class LowPassFilter : public DigitalFilter<double> {
 public:
   ...
   *             @f$ \tau_c = \frac{1}{2 pi f_c}@f$  where @f$ f_c @f$ is the
   * cutoff frequency
   */
  LowPassFilter(double idt, double omega_c, double ioutput = 0)
    ...
    if (omega_c < idt) {
      throw std::domain_error(
          "LowPassFilter constructor error: tua_c is smaller than the sample "
          "time dt.");
    }
  }
  /**
  ...
  double update(double newValue) final {
    return output = (output - newValue) * epow + newValue;
  }
  /**
...
class LowPassFilter2 : public DigitalFilter<double> {
 public:
  ...
  LowPassFilter2(double dt, double tau_c, double ioutput = 0)
      : yc{-2 * (pow(dt, 2) - 4 * pow(tau_c, 2)) /
               (pow(dt, 2) + 2 * sqrt(2) * tau_c * dt + 4 * pow(tau_c, 2)),
           (-pow(dt, 2) + 2 * sqrt(2) * tau_c * dt - 4 * pow(tau_c, 2)) /
               (pow(dt, 2) + 2 * sqrt(2) * tau_c * dt + 4 * pow(tau_c, 2))},
        xc{pow(dt, 2) /
               (pow(dt, 2) + 2 * sqrt(2) * tau_c * dt + 4 * pow(tau_c, 2)),
  ...
  double update(double newValue) final {
    x.push(newValue);
    double output = 0;
    for (int i = 0; i < 2; ++i) output += y.get(i) * yc[i];
    for (int i = 0; i < 3; ++i) output += x.get(i) * xc[i];
...
class HighPassFilter3 : public DigitalFilter<double> {
 public:
  HighPassFilter3(double sampleTime, double omega_c, double ioutput = 0)
      : xc{8, -24, 24, -8},
        yc{1 / (1 * tps::pow(sampleTime * omega_c, 3) +
                4 * tps::pow(sampleTime * omega_c, 2) +
                8 * sampleTime * omega_c + 8),
           3 * tps::pow(sampleTime * omega_c, 3) +
               4 * tps::pow(sampleTime * omega_c, 2) -
               8 * sampleTime * omega_c - 24,
           3 * tps::pow(sampleTime * omega_c, 3) -
               4 * tps::pow(sampleTime * omega_c, 2) -
               8 * sampleTime * omega_c + 24,
...
class LowPassFilter3 : public DigitalFilter<double> {
 public:
  LowPassFilter3(long double sampleTime, long double omega_c,
                 long double ioutput = 0)
      : yc{1,
           ...
           (double)((1 * tps::pow(sampleTime * omega_c, 3) -
                    ...
                    (1 * tps::pow(sampleTime * omega_c, 3) +
                     4 * tps::pow(sampleTime * omega_c, 2) +
                     8 * sampleTime * omega_c + 8))},
        xc{(double)(1 * tps::pow(sampleTime * omega_c, 3) /
                    (1 * tps::pow(sampleTime * omega_c, 3) +
                     4 * tps::pow(sampleTime * omega_c, 2) +
                     8 * sampleTime * omega_c + 8)),
           (double)(3 * tps::pow(sampleTime * omega_c, 3) /
...
class LowPassFilter3MatchedZ : public DigitalFilter<double> {
 public:
  LowPassFilter3MatchedZ(long double sampleTime, long double omega_c)
      : amplFac(-(2 *
                      (expl(3 * omega_c * sampleTime) -
                       expl(2 * omega_c * sampleTime)) *
                      cosl(sqrtl(3) * omega_c * sampleTime / 2) -
                  expl(7 * omega_c * sampleTime / 2) +
                  expl(3 * omega_c * sampleTime / 2)) *
                expl(-7 * omega_c * sampleTime / 2)),
        yc{(double)(-(2 * cosl(sqrtl(3) * omega_c * sampleTime / 2) *
                          expl(omega_c * sampleTime * 5 / 2) +
                      expl(2 * omega_c * sampleTime)) *
                    expl(-3 * omega_c * sampleTime)),
           (double)((2 * cosl(sqrtl(3) * omega_c * sampleTime / 2) *
                         expl(omega_c * sampleTime * 3 / 2) +
                     expl(2 * omega_c * sampleTime)) *
                    expl(-3 * omega_c * sampleTime)),
           (double)(-expl(-2 * omega_c * sampleTime))} {
    if (omega_c / (2 * M_PI) < sampleTime) {
      throw std::domain_error(
          "LowPassFilter3MatchedZ constructor error: tua_c is smaller than the "
          "sample time dt.");
    }
  }
  double update(double newValue) final {
...
class LowPassFilter3DiffApprox : public DigitalFilter<double> {
 public:
  LowPassFilter3DiffApprox(double sampleTime, double omega_c,
                           double ioutput = 0)
      : xc{1 * tps::pow(sampleTime * omega_c, 3), 0, 0, 0},
        yc{1 / (1 * tps::pow(sampleTime * omega_c, 3) +
                2 * tps::pow(sampleTime * omega_c, 2) +
                2 * sampleTime * omega_c + 1),
           0 * tps::pow(sampleTime * omega_c, 3) -
               2 * tps::pow(sampleTime * omega_c, 2) -
               4 * sampleTime * omega_c - 3,
           0 * tps::pow(sampleTime * omega_c, 3) +
               0 * tps::pow(sampleTime * omega_c, 2) +
               2 * sampleTime * omega_c + 3,
           0 * tps::pow(sampleTime * omega_c, 3) -
               0 * tps::pow(sampleTime * omega_c, 2) +
               0 * sampleTime * omega_c - 1} {
    if (omega_c < sampleTime) {
      throw std::domain_error(
          "LowPassFilter constructor error: tua_c is smaller than the sample "
          "time dt.");
