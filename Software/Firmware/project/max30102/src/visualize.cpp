...
void readCSV(const std::string& filename, std::vector<double>& x,
             std::vector<double>& y) {
  ...
  while (std::getline(file, line)) {
    ...
    char comma;
    ss >> x_value >> comma >> y_value;  // Assuming the CSV file has two values
...
int main() {
  ...
  std::string filename =
      "project/max30102/Sample Data/data_9.dat";  // Change to your CSV file's
                                                  // name
  ...
  while (true) {
    ...
    for (size_t i = 0; i < y_data.size(); ++i) {
      ...
      y_data[i] += 1;  // Increment y value by 1 (replace this with your desired
                       // update logic)
